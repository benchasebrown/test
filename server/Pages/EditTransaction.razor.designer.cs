...
namespace PersonalFinance.Pages
{
    public partial class EditTransactionComponent : ComponentBase
    {
        ...
        {
            var sqlGetTransactionBytransactionIdResult = await Sql.GetTransactionBytransactionId(transaction_id);
            transaction = sqlGetTransactionBytransactionIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(PersonalFinance.Models.Sql.Transaction args)
        {
            try
            {
                var sqlUpdateTransactionResult = await Sql.UpdateTransaction(transaction_id, transaction);
                DialogService.Close(transaction);
            }
            catch (System.Exception sqlUpdateTransactionException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Transaction" });
            }
        }
