...
{
    public partial class AddBillComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            bill = new PersonalFinance.Models.Sql.Bill(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(PersonalFinance.Models.Sql.Bill args)
        {
            try
            {
                var sqlCreateBillResult = await Sql.CreateBill(bill);
                DialogService.Close(bill);
            }
            catch (System.Exception sqlCreateBillException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Bill!" });
            }
        }
